...
class InstructorNavbar extends Component {
    ...
    retrieveUser(username){
        axios.get("/api/v1/instructor/findByUsername/"+username,{headers:authHeader()})
            .then(response => {
                this.setState({
    ...
    retrieveUnreadNotificationsLength=(username)=>{
        axios.get("/api/v1/notification/retrieveUnreadLength/instructor/" + username, {headers: authHeader()})
            .then(response => {
           ...
           <nav style={{background: "#380B61"}} className="navbar navbar-collapse navbar-dark">
                ...
                <div className="navbar-nav navbar-expand navbar-center">
                    ...
                    <li className="nav-item mx-5">
                        <Link to={"/instructor/new-assignment"} className="nav-link">
                              New Assignment
                        </Link>
                    </li>
                    <li className="nav-item mx-5">
                        <Link to={"/instructor/all-assignments"} className="nav-link">
                              Uploaded Assignments
                        </Link>
                    </li>
                    <li className="nav-item mx-5">
                        <Link to={"/instructor/class-list"} className="nav-link">
                              Class List
                        </Link>
                    </li>
                    <li className="nav-item mx-5">
                    ...
                    <li className="nav-item mx-5">
                        <Link to={"/instructor/notification"}  className="nav-link">
